document.addEventListener("DOMContentLoaded", () => {
  ...
  function original(){
    return shell(ruby(data.title),"先掌握論述：動畫發展 → 漫畫產業 → 表現手法 → 世界品牌。",`<article class="reading-paper" style="--reading-size:${[.98,1.1,1.24][state.font]}rem"><div class="reading-badge">第14課</div>${data.paragraphs.map(p=>`<p data-paragraph="${p.id}">${ruby(p.jp)}</p>`).join("")}<footer>（${esc(data.author)}）</footer></article>`);
  }
