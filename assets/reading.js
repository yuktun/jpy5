document.addEventListener("DOMContentLoaded", () => {
  ...
  function original(){
    return shell(ruby(data.title),"先掌握時間線：來日初期 → 活動範圍擴大 → 在日第六年解開誤會。",`<article class="reading-paper" style="--reading-size:${[.98,1.1,1.24][state.font]}rem"><div class="reading-badge">第13課</div>${data.paragraphs.map(p=>`<p data-paragraph="${p.id}">${ruby(p.jp)}</p>`).join("")}<footer>（${esc(data.author)}）</footer></article>`);
  }
  ...
  function questionCards(){
    return shell("閱讀問題","每題作答後即時顯示解釋，答錯會加入錯題重溫。",`<div class="reading-question-list">${data.questions.slice(0,6).map((q,i)=>`<article class="reading-question" data-question="${q.id}"><span>問題 ${i+1}</span><h3>${ruby(q.q)}</h3><div>${q.options.map((o,n)=>`<button data-q="${q.id}" data-option="${n}">${ruby(o)}</button>`).join("")}</div><p class="reading-feedback"></p></article>`).join("")}</div>`);
  }
