window.JPY5 = (() => {
  ...
  function initLessonSwitchers(){document.querySelectorAll(".module-lesson-switcher,.reading-controls .lesson-pills").forEach(nav=>{const module=(location.pathname.match(/chapter-(?:13|14|15|16|17)-([^./]+)\.html/)||[])[1];if(!module)return;const lesson=(location.pathname.match(/chapter-(\d+)-/)||[])[1];nav.innerHTML=[13,14,15,16,17].map(n=>`<a class="${String(n)===lesson?"active":""}" ${String(n)===lesson?"aria-current=\"page\"":""} href="chapter-${n}-${module}.html">第${n}課</a>`).join("")+`<span class="lesson-pill-disabled" aria-disabled="true">第18課<br><small>準備中</small></span>`})} document.addEventListener("DOMContentLoaded",()=>{initLessonSwitchers();
