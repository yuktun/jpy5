document.addEventListener("DOMContentLoaded", () => {
  ...
  const dialogueTarget = x => segments(x).map(part => part.type === "fixed"
    ? `<span class="focus-fixed">${ruby(part.text)}</span>`
    : `<button class="focus-line" data-focus-line="${x.id}" data-answer-text="${esc(part.text)}" type="button" aria-haspopup="dialog" aria-label="查看底線句解釋">${ruby(part.text)}</button>`).join("");
  ...
  function renderChoice() {
    ...
    stage.querySelectorAll("[data-answer]").forEach(b => b.onclick = () => {
      ...
      stage.querySelector(".answer-message").innerHTML = ok ? `答對了。${esc(x.use)}` : `正確答案：${ruby(x.jp)}`;
  ...
  function renderComprehension() {
    stage.innerHTML = `<div class="stage-heading"><div><p class="kicker">教材問題</p><h2>內容與表現理解</h2></div><span>6 題</span></div><div class="reading-question-list">${data.comprehension.map((q,index) => {
  ...
  function renderOrder() {
    ...
    stage.innerHTML = `<div class="stage-heading"><div><p class="kicker">句子重組</p><h2>砌出完整底線句</h2></div><span>${current + 1} / ${data.items.length}</span></div><article class="practice-panel">${listenButton(x.id,"聽一次提示")}<p class="focus-meaning">${esc(x.zh)}</p><div class="sentence-build">${built.length ? built.map((c,i) => `<button data-remove="${i}">${ruby(c)}</button>`).join("") : '<span>點下面詞塊組成句子</span>'}</div><div class="word-bank">${orderPool.map((c,i) => `<button data-chunk="${i}">${ruby(c)}</button>`).join("")}</div><div class="practice-actions"><button class="text-button" id="clear-build">重新開始</button><button class="primary-button" id="check-build">檢查答案</button></div><p class="answer-message"></p></article>`;
  ...
  function renderMistakes() {
    ...
    stage.innerHTML = `<div class="stage-heading"><div><p class="kicker">錯題重溫</p><h2>針對容易錯的內容</h2></div><span>${total} 項待重溫</span></div>${total ? `<div class="mistake-list">${wrong.map(x => `<article><span>句子練習 · 錯過 ${state.mistakes[x.id]} 次</span><h3>${ruby(x.jp)}</h3><p>${esc(x.zh)}</p>${listenButton(x.id)}<button class="chip" data-mastered="${x.id}">標記已掌握</button></article>`).join("")}${comprehensionWrong.map(q=>`<article><span>${q.kind} · 錯過 ${state.comprehensionMistakes[q.id]} 次</span><h3>${embeddedRuby(q.q)}</h3><p>答案：${rubyIfJapanese(q.options[q.answer])}</p><p>${embeddedRuby(q.why)}</p><button class="chip" data-mastered-comprehension="${q.id}">標記已掌握</button></article>`).join("")}</div>` : `<div class="empty-state"><h2>暫時沒有錯題</h2><p>完成教材問題、選擇題、重組或默寫後，錯題會自動來到這裡。</p><button class="primary-button" data-go-choice>開始選擇題 →</button></div>`}`;
