document.addEventListener("DOMContentLoaded", () => {
  ...
  function original(){
    return shell(ruby(data.title),"按時間線掌握：資料外洩 → 會員投訴 → 公司調查與對策 → 受害者個案。",`<article class="reading-paper" style="--reading-size:${[.98,1.1,1.24][state.font]}rem"><div class="reading-badge">第16課</div>${data.paragraphs.map(p=>`<p data-paragraph="${p.id}">${ruby(p.jp)}</p>`).join("")}<footer>（${esc(data.author)}）</footer></article>`);
  }
