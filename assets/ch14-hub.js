document.addEventListener("DOMContentLoaded", () => {
  ...
  set("vocabulary", `${vocabDone}/208 已評估`);
  set("grammar", `${grammarDone}/57 張已評估`);
  set("reading", `${readingDone}/9 題已完成`);
  set("listening", `${listeningDone}/6 題已完成`);
  set("quiz", `${quizHistory.length} 次測驗紀錄`);
  const milestones = [vocabDone/208, grammarDone/57, readingDone/9, listeningDone/6, Math.min(quizHistory.length,1)].map(value=>Math.min(1,value));
