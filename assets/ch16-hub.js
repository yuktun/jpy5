document.addEventListener("DOMContentLoaded", () => {
  ...
  set("vocabulary", `${vocabDone}/185 已評估`);
  set("grammar", `${grammarDone}/50 張已評估`);
  set("reading", `${readingDone}/9 題已完成`);
  set("listening", `${listeningDone}/7 題已完成`);
  set("quiz", `${quizHistory.length} 次測驗紀錄`);
  const milestones = [vocabDone/185, grammarDone/50, readingDone/9, listeningDone/7, Math.min(quizHistory.length,1)].map(value=>Math.min(1,value));
