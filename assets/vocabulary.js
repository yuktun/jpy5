...
const sectionLabel = item => sections[item.section]?.label || "第 13 課";
...
function renderCard() {
  ...
  const sameForm = frontText(item) === backTitle(item);
   $("#vocab-card-back").innerHTML = `<p>題目 · ${frontLabel()}</p><h2 class="card-retained-prompt" lang="ja">${frontText(item)}</h2><span class="card-answer-label">答案 · ${state.direction === "normal" ? scriptLabel(item) : "平假名／片假名"}</span>${sameForm ? `<span class="same-form-answer">✓ 讀音、寫法相同</span>` : `<h2 class="card-revealed-answer" lang="ja">${backTitle(item)}</h2>`}${!sameForm && item.original ? `<span class="vocab-writing">日文表記 · ${item.written}</span>` : ""}<strong>${item.meaning}</strong><span>${item.type}${item.note ? ` · ${item.note}` : ""}</span>`;
...
$("#reset-vocabulary").onclick = () => { if(confirm("清除全部 159 詞的記憶卡進度、答題標記和收藏？")){state=structuredClone(defaults);renderCard();} };
