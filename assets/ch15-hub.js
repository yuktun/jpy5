document.addEventListener("DOMContentLoaded", () => {
  ...
  set("vocabulary", `${vocabDone}/146 已評估`);
  set("grammar", `${grammarDone}/41 張已評估`);
  set("reading", `${readingDone}/7 題已完成`);
  set("listening", `${listeningDone}/6 題已完成`);
  set("quiz", `${quizHistory.length} 次測驗紀錄`);
  const milestones = [vocabDone/146, grammarDone/41, readingDone/7, listeningDone/6, Math.min(quizHistory.length,1)].map(value=>Math.min(1,value));
