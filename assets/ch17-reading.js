document.addEventListener("DOMContentLoaded", () => {
  ...
  function original(){
    return shell(ruby(data.title),"閱讀〈暦〉：從月份名稱、舊曆到明治改曆。",`<article class="reading-paper" style="--reading-size:${[.98,1.1,1.24][state.font]}rem"><div class="reading-badge">第17課</div>${data.paragraphs.map(p=>`<p data-paragraph="${p.id}">${ruby(p.jp)}</p>`).join("")}<footer>（${ruby(data.author)}）</footer></article>`);
  }
